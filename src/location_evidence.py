...
def init_evidence(conn):
    ...
    columns = {r[1] for r in conn.execute('PRAGMA table_info(address_evidence)')}
    for name in ('source_row_hash', 'text_sha256'):
        if name not in columns:
            conn.execute(f'ALTER TABLE address_evidence ADD COLUMN {name} TEXT')


def save_evidence(conn, doc_id, candidates, source_row_hash=None, text_sha256=None):
    conn.execute('''INSERT OR REPLACE INTO address_evidence
                    (doc_id,version,candidates,source_row_hash,text_sha256) VALUES (?, ?, ?, ?, ?)''',
                 (doc_id, addr.EXTRACTION_VERSION,
                  json.dumps(candidates, ensure_ascii=False), source_row_hash, text_sha256))
...
def load_candidates(conn, doc_id):
    ...
    if not row or row[0] != addr.EXTRACTION_VERSION:
        return []
    return parse_candidates(row[1]) or []


def parse_candidates(payload):
    """None означає пошкоджений результат; [] — коректну відсутність адрес."""
    try:
        items = json.loads(payload) if isinstance(payload, str) else payload
        if not isinstance(items, list):
            return None
        candidates = [addr.AddressCandidate(**item) for item in items]
        for c in candidates:
            if (any(type(v) is not int for v in (c.start, c.end, c.context_start)) or
                    not isinstance(c.street, str) or not c.street or
                    c.house is not None and not isinstance(c.house, str) or
                    not isinstance(c.context, str) or not isinstance(c.raw, str) or
                    c.context_start < 0 or c.end <= c.start or
                    c.role not in ('EVENT_LOCATION', 'COURT', 'RESIDENCE', 'WORKPLACE',
